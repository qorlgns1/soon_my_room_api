...
class S3StorageServiceTest {
  ...
  void setUp() throws IOException {
    ...
    validImageFile =
        new MockMultipartFile("image", "test-image.jpg", "image/jpeg", validImageContent);

    // 크기가 큰 이미지 파일 (11MB)
    byte[] largeImageContent = new byte[11 * 1024 * 1024];
    Arrays.fill(largeImageContent, (byte) 1);
    largeImageFile =
        new MockMultipartFile("image", "large-image.jpg", "image/jpeg", largeImageContent);
    ...
    invalidExtensionFile =
        new MockMultipartFile(
            "file", "test-document.pdf", "application/pdf", invalidExtensionContent);
  ...
  void determineBucket_ProfileImage() {
    ...
    assertEquals(
        testProfilesBucket,
        ReflectionTestUtils.invokeMethod(s3StorageService, "determineBucket", profileRequestPath));
    assertEquals(
        testProfilesBucket,
        ReflectionTestUtils.invokeMethod(s3StorageService, "determineBucket", userRequestPath));
  }
  ...
  void determineBucket_PostImage() {
    ...
    assertEquals(
        testPostsBucket,
        ReflectionTestUtils.invokeMethod(s3StorageService, "determineBucket", postRequestPath));
  }
  ...
  void determineBucket_ProductImage() {
    ...
    assertEquals(
        testProductsBucket,
        ReflectionTestUtils.invokeMethod(s3StorageService, "determineBucket", productRequestPath));
  }
  ...
  void determineBucket_DefaultBucket() {
    ...
    assertEquals(
        testDefaultBucket,
        ReflectionTestUtils.invokeMethod(s3StorageService, "determineBucket", unknownRequestPath));
    assertEquals(
        testDefaultBucket,
        ReflectionTestUtils.invokeMethod(s3StorageService, "determineBucket", nullRequestPath));
  }
